...
async function main() {
    ...
    for (const usuario of usuarios) {
        ...
        if (botGeneratedTotal <= 0) {
            console.log(`  ℹ️  ${nombre}: ${currentTotal} min (todo es Botrix), sin cambios`);
            sinCambios++;
            continue;
        }
        ...
        if (totalWindowMinutes <= 0) {
            console.log(`  ⚠️  ${nombre}: ventana de tiempo inválida (${totalWindowMinutes} min)`);
            errores++;
            continue;
        }

        // El watchtime generado por el bot no puede exceder el tiempo real de actividad
        const correctedBotGenerated = Math.min(botGeneratedTotal, totalWindowMinutes);
        ...
        console.log(`      Bot gen: ${botGeneratedTotal} → ${correctedBotGenerated} min`);
        console.log(`      Ventana: ${totalWindowMinutes} min (${firstMsg.toISOString()} → ${lastMsg.toISOString()})`);
    ...
    if (DRY_RUN && cambios > 0) {
        console.log('');
        console.log('⚠️  Este fue un DRY-RUN. Ejecuta sin --dry-run para aplicar cambios.');
    }
