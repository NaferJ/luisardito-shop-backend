...
async function uploadKickAvatarToCloudinary(
  ...
  try {
    ...
    if (!cloudinary) {
      logger.warn(
        `[Upload Avatar] Cloudinary not available, using original Kick URL`
      );
    ...
    if (kickAvatarUrl.includes("/img/default-profile-pictures/")) {
      logger.info(
        `[Upload Avatar] Kick default avatar detected, using original URL`
      );
      return kickAvatarUrl; // Use original URL for default avatars
    }

    logger.info(
      `[Upload Avatar] Downloading Kick avatar for user ${userId}:`,
      kickAvatarUrl
    );
    ...
    logger.info(
      `[Upload Avatar] Uploading to Cloudinary for user ${userId}...`
    );
...
export { uploadKickAvatarToCloudinary };
