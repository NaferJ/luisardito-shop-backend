...
async function generatePromotionsPDF(
  ...
  return new Promise((resolve, reject) => {
    try {
      logger.info(
        `[PDF Generator] Generating PDF with ${promociones.length} promotions`
      );
      ...
      doc
        .fontSize(10)
        .font("Helvetica")
        .text(`Generated on: ${new Date().toLocaleString("es-MX")}`, {
          align: "center",
        });
      ...
      doc
        ...
        .text(
          `Points deducted: ${totalPuntosDescontados.toLocaleString("es-MX")} pts`
        );
      ...
      logger.info(
        `[PDF Generator] Promotions to process: ${promociones.length}`
      );
      ...
      if (promociones.length === 0) {
        ...
        promociones.forEach((promo, index) => {
          ...
          doc
            ...
            .font("Helvetica")
            .fillColor("#000000")
            .text(nombre.substring(0, 25), col1, currentY, {
              width: 140,
              ellipsis: true,
            });
