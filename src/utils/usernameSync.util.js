...
async function syncUsernameIfNeeded(usuario, kickUsername, kickUserId, forceSync = false) {
    try {
        // DEBUG: Log al inicio
        logger.debug(`[DEBUG SYNC] Starting sync for ${kickUserId}: current '${usuario?.nickname}' vs new '${kickUsername}'`);
        ...
        logger.info(`[Username Sync] Change detected: "${usuario.nickname}" -> "${kickUsername}" (ID: ${kickUserId})`);
        ...
        if (!forceSync) {
            try {
                ...
                if (lastSync) {
                    ...
                    logger.info(`[Username Sync] Last sync ${hoursSinceSync.toFixed(1)}h ago - Throttling active (24h)`);
                    logger.debug(`[DEBUG SYNC] Return: Throttling active (${hoursSinceSync.toFixed(1)}h)`);
                    return { 
                        updated: false, 
                ...
                logger.warn(`[Username Sync] Error in Redis, continuing without throttling:`, redisError.message);
        ...
        if (colision) {
            logger.warn(`[Username Sync] COLLISION: "${kickUsername}" already exists (user ID: ${colision.id})`);
        ...
        logger.info(`[Username Sync] User ID ${usuario.id} updated: "${oldNickname}" -> "${kickUsername}"`);
        logger.debug(`[DEBUG SYNC] Update successful: "${oldNickname}" -> "${kickUsername}"`);
        ...
        logger.info(`[Username Sync] Throttling activated for 24h for user ${kickUserId}`);
        logger.debug(`[DEBUG SYNC] Throttling saved in Redis`);
            } catch (redisError) {
        logger.warn(`[Username Sync] Error saving throttling in Redis:`, redisError.message);
        ...
        logger.error(`[Username Sync] Error syncing username:`, error.message);        logger.debug(`[DEBUG SYNC] General error: ${error.message}`);        return {
            updated: false, 
...
async function syncUserProfileIfNeeded(usuario, kickUsername, kickUserId, forceSync = false, kickProfilePicture = null) {
    try {
        // DEBUG: Log al inicio
        logger.debug(`[DEBUG SYNC PROFILE] Starting full sync for ${kickUserId}: current '${usuario?.nickname}' vs new '${kickUsername}'`);
        ...
        if (usuario.nickname !== kickUsername) {
        logger.info(`[Profile Sync] Username change detected: "${usuario.nickname}" -> "${kickUsername}" (ID: ${kickUserId})`);
        ...
        return { updated: false, reason: 'Profile unchanged', usernameChanged: false, avatarChanged: false };
        }
        ...
        if (!forceSync) {
            try {
                ...
                if (lastSync) {
                    ...
                    logger.info(`[Profile Sync] Last sync ${hoursSinceSync.toFixed(1)}h ago - Throttling active (24h)`);
                    logger.debug(`[DEBUG SYNC PROFILE] Return: Throttling active (${hoursSinceSync.toFixed(1)}h)`);
                    return {
                        updated: false,
                        reason: `Throttling: last sync ${hoursSinceSync.toFixed(1)}h ago`,
                ...
                logger.warn(`[Profile Sync] Error in Redis, continuing without throttling:`, redisError.message);
                logger.debug(`[DEBUG SYNC PROFILE] Redis error, continuing without throttling`);
            }
        }
        ...
        if (usernameChanged) {
            ...
            if (colision) {
                logger.warn(`[Profile Sync] COLLISION: "${kickUsername}" already exists (user ID: ${colision.id})`);
                logger.debug(`[DEBUG SYNC PROFILE] Return: Collision with user ${colision.id}`);
                return {
                    updated: false,
                    reason: `Collision: name already used by user ID ${colision.id}`,
                    usernameChanged: false,
        ...
        logger.info(`[Profile Sync] User ID ${usuario.id} updated: ${usernameChanged ? `"${oldNickname}" -> "${kickUsername}"` : 'username unchanged'}${avatarChanged ? ', avatar updated' : ''}`);
        logger.debug(`[DEBUG SYNC PROFILE] Update successful: username=${usernameChanged}, avatar=${avatarChanged}`);
        ...
        if (!forceSync) {
            try {
                ...
                const syncKey = `profile_sync:${kickUserId}`;
                await redis.set(syncKey, new Date().toISOString(), 'EX', 86400); // 24 hours
        logger.info(`[Profile Sync] Throttling activated for 24h for user ${kickUserId}`);
        logger.debug(`[DEBUG SYNC PROFILE] Throttling saved in Redis`);
            } catch (redisError) {
        logger.warn(`[Profile Sync] Error saving throttling in Redis:`, redisError.message);
