...
exports.crear = async (req, res) => {
  ...
  try {
    ...
    if (valor_descuento < 0) {
      return res
        .status(400)
        .json({ error: "El valor del descuento no puede ser negativo" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Error creating the promotion" });
    res
      .status(500)
      .json({ error: error.message || "Error creating the promotion" });
  }
...
exports.actualizar = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Error updating the promotion" });
    res
      .status(500)
      .json({ error: error.message || "Error updating the promotion" });
  }
...
exports.obtenerEstadisticas = async (req, res) => {
  try {
    const { id } = req.params;
    const estadisticas =
      await promocionService.obtenerEstadisticasPromocion(id);
...
exports.asignarProductos = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error assigning products to the promotion" });
  }
};
