...
exports.me = async (req, res) => {
  ...
  // 🔍 DEBUG: Verificar qué devuelve la API de Kick AHORA para este usuario
  if (user.user_id_ext) {
    try {
      logger.info("🔍 [DEBUG AVATAR] Consultando API de Kick para usuario:", {
        nickname: user.nickname,
        kick_user_id: user.user_id_ext,
      });
      ...
      if (broadcasterToken && broadcasterToken.access_token) {
        logger.info("🔍 [DEBUG AVATAR] Token encontrado, consultando API...");
        ...
        const apiResponse = await axios
          ...
          .catch((err) => {
            logger.warn(
              "🔍 [DEBUG AVATAR] Error llamando a API de Kick:",
              err.message,
            );
            return null;
          });
        ...
        if (apiResponse && apiResponse.data) {
          logger.info("🔍 [DEBUG AVATAR] ===== RESPUESTA DE KICK API =====");
          logger.info("🔍 [DEBUG AVATAR] Usuario:", user.nickname);
          logger.info("🔍 [DEBUG AVATAR] Kick User ID:", user.user_id_ext);
          logger.info(
            "🔍 [DEBUG AVATAR] Respuesta completa:",
            JSON.stringify(apiResponse.data, null, 2),
          );

          const userData = apiResponse.data?.data?.[0] || apiResponse.data;
          if (userData) {
            logger.info(
              "🔍 [DEBUG AVATAR] profile_picture devuelto:",
              userData.profile_picture,
            );
            logger.info(
              "🔍 [DEBUG AVATAR] Avatar actual en BD:",
              user.kick_data?.avatar_url,
            );

            if (userData.profile_picture !== user.kick_data?.avatar_url) {
              logger.warn(
                "🔍 [DEBUG AVATAR] ⚠️ DIFERENCIA DETECTADA entre API y BD!",
              );
              logger.warn(
                "🔍 [DEBUG AVATAR] API dice:",
                userData.profile_picture,
              );
              logger.warn(
                "🔍 [DEBUG AVATAR] BD tiene:",
                user.kick_data?.avatar_url,
              );
            } else {
              logger.info("🔍 [DEBUG AVATAR] ✅ Avatar en BD coincide con API");
            }
          }
          logger.info(
            "🔍 [DEBUG AVATAR] =====================================",
        ...
        logger.info(
          "🔍 [DEBUG AVATAR] No hay token guardado para este usuario",
        );
      }
    } catch (error) {
      logger.error(
        "🔍 [DEBUG AVATAR] Error en debug de avatar:",
        error.message,
      );
    }
  }
