...
const authorize = (allowedRoles) => {
  return async (req, res, next) => {
    try {
      ...
      if (!req.user) {
        logger.warn(
          "[Authorize] Usuario no autenticado intentando acceder a ruta protegida"
        );
      ...
      if (!userRol) {
        logger.error(
          `[Authorize] Rol no encontrado para usuario ${req.user.nickname} (rol_id: ${req.user.rol_id})`
        );
      ...
      if (hasPermission) {
        logger.info(
          `[Authorize] ✅ Usuario ${req.user.nickname} (${userRoleName}) autorizado`
        );
        return next();
      }
      ...
      logger.warn(
        `[Authorize] ❌ Usuario ${req.user.nickname} (${userRoleName}) sin permisos. Se requiere: ${allowedRoles.join(", ")}`
      );
