...
class DbCleanupService {
  ...
  static async runAll() {
    ...
    try {
      ...
      logger.info(
        "[DB-CLEANUP] kick_webhook_events: " +
          results.webhookEvents.deleted_processed +
          " processed deleted, " +
          results.webhookEvents.deleted_very_old +
          " very old deleted"
      );
      results.historialPuntos = await this.cleanHistorialPuntos();
      logger.info(
        "[DB-CLEANUP] historial_puntos: " +
          results.historialPuntos.nullified_json +
          " JSONs cleaned, " +
          results.historialPuntos.deleted_chat_records +
          " chat records deleted"
      );
      results.notificaciones = await this.cleanNotificaciones();
      logger.info(
        "[DB-CLEANUP] notificaciones: " +
          results.notificaciones.deleted_read +
          " read deleted, " +
          results.notificaciones.deleted_old_unread +
          " old deleted"
      );
      results.refreshTokens = await this.cleanRefreshTokens();
      logger.info(
        "[DB-CLEANUP] refresh_tokens: " +
          results.refreshTokens.deleted_revoked_expired +
...
export default DbCleanupService;
