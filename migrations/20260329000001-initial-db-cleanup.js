...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    console.log(
      "🧹 [MIGRATION] Esto puede tomar varios minutos si hay muchos registros.\n"
    );
    // ═══════════════════════════════════════════════════════════
    ...
    console.log(
      "2/6 - Limpiando kick_webhook_events (no procesados > 30 dias)..."
    );
    let totalOld = 0;
    do {
      [, batchDeleted] = await queryInterface.sequelize.query(
    ...
    // ═══════════════════════════════════════════════════════════
    console.log(
      "3/6 - Limpiando kick_event_data JSON en historial_puntos (> 30 dias)..."
    );
    ...
    console.log(
      "   -> " + (nullMeta?.affectedRows || 0) + " registros con JSON limpiado"
    );
    // Eliminar registros de chat > 90 dias (en batches)
    console.log(
      "4/6 - Eliminando registros de chat en historial_puntos (> 90 dias)..."
    );
    let totalChat = 0;
    do {
      [, batchDeleted] = await queryInterface.sequelize.query(
    ...
    console.log(
      "   -> " +
        (readMeta?.affectedRows || 0) +
        " leidas + " +
        (unreadMeta?.affectedRows || 0) +
        " antiguas eliminadas"
    );
    // ═══════════════════════════════════════════════════════════
    ...
    console.log(
      "   -> " + (tokenMeta?.affectedRows || 0) + " tokens eliminados"
    );
  ...
  async down(queryInterface, Sequelize) {
    ...
    console.log(
      "Esta migracion no es reversible (limpieza de datos temporales)"
    );
  },
};
